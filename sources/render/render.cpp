	...
	for ( u32 i = 0; i < max_frame_delay; ++i )
	{
		for ( u32 j = 0; j < max_frame_delay; ++j )
		{
...

			{
				...
				barriers[0].Transition.pResource = g_resources.constant_buffer( j );
				...
				barriers[1].Transition.pResource = m_indirect_arguments_buffer;
				...
				barriers[2].Transition.pResource = m_instance_transforms_buffer;
...

			{
				...
				u32 const dispatch_size_offset = offsetof( gpu::constant_buffer, indirect_params.x );
...

			{
				...
				u32 const instance_count_offset = offsetof( gpu::constant_buffer, indirect_params.w );
...
void render::destroy( )
{
	...
	resource_system::destroy_resources(
		...
		&m_ps_generate_mesh_arguments, &m_ps_render_mesh_objects, &m_ps_shade
	);
}
...
{
	{
		...
		while ( !m_copy_finished || ( m_gpu_frame_index->GetCompletedValue( ) < m_frame_index ) )
		{
	...
	if ( m_scene != nullptr )
	{
		...
		u32 const frame_list_index = ( m_frame_index + 1 ) % max_frame_delay;
...
void render::prepare_frame( )
{
	...
	if ( m_scene )
	{
		...
		copy_tasks[task_index].set_buffer_upload( m_mesh_object_list[cmd_index], 0 );
		copy_tasks[task_index].set_source_data( cpu_mesh_object_list.data( ), mesh_list_size * sizeof(u32) );
		copy_tasks[task_index].set_task_context( context );
		++task_index;
	}
	...
	cpu_constant_buffer.indirect_params = math::u32x4( ( mesh_list_size + 255 ) / 256, 1, 1, mesh_list_size );

	copy_tasks[task_index].set_buffer_upload( g_resources.constant_buffer( cmd_index ), 0 );
	copy_tasks[task_index].set_source_data( &cpu_constant_buffer, sizeof(gpu::constant_buffer) );
	copy_tasks[task_index].set_task_context( context );
	++task_index;
	...
	{
		g_resources.update_dynamic_transforms_task( copy_tasks[task_index] );
		copy_tasks[task_index].set_task_context( context );
		++task_index;
	}
	...
	g_gpu_uploader.push_immediate_tasks( copy_tasks, copy_task_count );
}
